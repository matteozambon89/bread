 ...
 * (and similar container hosts) pass `{ host: '0.0.0.0', port: 8080 }` — those
 * are not the defaults here, so local use stays aligned with Bun `startServer`.
 *
...
export async function startServerNode(
  ...
  if (idleTimeout !== undefined) {
    ...
    ;(server as HttpServer).timeout = idleTimeout * 1000
  }
